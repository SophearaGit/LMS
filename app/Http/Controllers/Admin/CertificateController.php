...
class CertificateController extends Controller
{
    public function download(Course $course)
    {
        ...
        $certificateRecord = Certificate::firstOrCreate(
            [
                'user_id' => Auth::id(),
                'course_id' => $course->id,
            ],
            [
                'certificate_no' => 'CAIT-' . strtoupper(uniqid()),
                'issued_at' => now(),
            ]
        );
        // Increase download count every time the PDF is downloaded
        $certificateRecord->increment('download_count');
        $data = [
